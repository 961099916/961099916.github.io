...
- 使用的是 linux 系统，所以安装方法：终端中输入：sudo apt-get install git，老一点的版本则需要运行命令：sudo apt-get install git-core，也可以源码安装则需要进行 make instal 。
...
1. 创建版本库：
   ...
   > 1. 先创建一个空目录：mkdir /git/learn_git
   > 2. 进入该目录：cd /git/learn_git
   > 3. 初始化该目录：git init

2. 提交项目

   > 1. 需要先把文件加入到 git 暂存区：git add 文件名
...
3.  版本回退：
    ...
    > 1. 通过 HEAD^ 一个^回退一个版本，命令：git reset --hard HEAD^
...
![git版本回退图片](https://imgconvert.csdnimg.cn/aHR0cDovL2ltZy5ibG9nLmNzZG4ubmV0LzIwMTgwMzExMjE1MTQwMTIz?x-oss-process=image/format,png)
     ...
     > 2. 通过commit的id回退，命令：git reset --hard  ad509b2a（只需要头几位就行，越多越容易找到，可以回到你知道id的版本）
...
![通过commit的id回退](https://imgconvert.csdnimg.cn/aHR0cDovL2ltZy5ibG9nLmNzZG4ubmV0LzIwMTgwMzExMjIwOTAxNjQ3?x-oss-process=image/format,png)
    ...
    > 3. 查询通过回退的版本号（commit id），命令：git reflog
...
1. 创建连接：
   ...
   > 1. 创建 SSH key。在用户目录下面查看.ssh 目录，如果里面没有则需要创建 SSH key，执行命令：ssh-keygen -t rsa -C "email@example.com"，生成两个文件 id-rsa.pub 和 id_rsa，id_rsa 是公钥，然后登录 github 输入公钥的内容进行连接。
   > 2. 添加仓库。
...
> 6. 添加冲突，先创建分支 dev 在 dev 分支中修改 readme.txt 然后提交 再切会 master 然后再修改 readme.txt 在提交，然后合并，就会遇到冲突，然后在单独修改冲突文件，重新再提交冲文件即可。
...
> 7. 保存 dev 分支的提交记录，执行命令：git merge --no-ff -m '合并信息' dev
...
> 8. Bug 分支，当在进行开发中遇到紧急修复 Bug，可以先把当前分支储藏起来执行命令:git stash，然后创建其他分支修复，修复后进行合并，然后再转换到该分支上执行命令:git stash pop，来进行回复。
> 9. 多人协作：
...
> 10. 当要提交时，如果遇到提交失败，则可以看看相关信息，然后 pull 下来，看是否需要创建远程 origin 的分支，进行 pull 下来会提示有冲突，当冲突修改之后在进行上传。
...
> 2. 在某次 commit 上打标签，则需要该 commit 的 id，执行命令：git tag 标签名 commit_id
