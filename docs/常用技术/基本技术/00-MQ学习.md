...
1. 拉取RabbitMQ的镜像执行命令 `docker pull rabbitmq:3.7-management` 
...
topic既可以实现direct，也可实现Fanout，topic通过routing_key进行路由，key可以存在两种特殊字符“ * ”与“#”，用于做模糊匹配，其中“*”用于匹配一个单词，“#”用于匹配多个单词（可以是零个）
*代表全部的
...
这里其他的service是用于记录RabbitMQ的日志，用于处理投递失败和消费失败的情况。
...
[RabbitMQ消息路由机制流程总结](https://blog.csdn.net/qq_27529917/article/details/79289564)
