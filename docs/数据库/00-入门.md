...
where SId in (
    select SId
    from (select SId, count(1) count from SC group by SId) t1
    where t1.count = 2
)
```
21. 查询男生、女生人数
...
```sql
select s1.Sname,count(1) from Student s1 ,Student s2 where s1.Sname = s2.Sname and s1.Ssex = s2.Ssex and s1.SId != s2.SId group by s1.Sname
```
24. 查询 1990 年出生的学生名单
