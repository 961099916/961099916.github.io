...
```sql
select * from (select SId,score from SC where CId in (select CId from Course where TId in (select TId from Teacher where Tname = "张三")) order by score desc limit 1) t1 left join Student s on t1.Sid = s.SId
```
34. 成绩有重复的情况下，查询选修「张三」老师所授课程的学生中，成绩最高的学生信息及其成绩
```sql
select * from (select SId,score from SC where score = (select score from SC where CId in (select CId from Course where TId in (select TId from Teacher where Tname = "张三")) order by score desc limit 1)) t1 left join Student s on t1.Sid = s.SId
```
